        ...
        while True:
            if self.fobj is None:
                raise WebSocketError(
                    'Connenction closed unexpectedly while'
...
class WebSocketHybi(object):
    ...
    def receive(self):
        ...
        while True:
            ...
            if opcode == self.OPCODE_TEXT:
                ...
                if length >= 2:
                    reason, message = struct.unpack(
                        '!H%ds' % (length - 2), buffer(payload)
                    )
                else:
                    reason = message = None
    ...
    def _is_valid_opcode(self, opcode):
        return opcode in (self.OPCODE_CONTINUATION, self.OPCODE_TEXT,
                          self.OPCODE_BINARY, self.OPCODE_CLOSE,
                          self.OPCODE_PING, self.OPCODE_PONG)
    ...
    def close(self, reason=1000, message=''):
        ...
        if self.fobj is not None:
            message = self._encode_text(message)
            self.send(
                struct.pack('!H%ds' % len(message), reason, message),
